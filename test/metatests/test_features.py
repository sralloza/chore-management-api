...
def test_validate_feature_name(api_@old
Feature: Feature):
...
def test_validate_common_scenarios(api_@old
Feature: Feature):
...
def test_validate_xflat_header_scenarios(api_@old
Feature: Feature):
    operation_id = get_operation_id_by_feature(api_feature)
    scenario_names = [scenario.name for scenario in api_feature.scenarios]
...
def test_scenarios_should_leave_two_break_lines(@old
Feature: Feature):
...
def test_2_spaces_instead_of_4(@old
Feature: Feature):
