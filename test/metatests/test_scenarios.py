...
def test_validate_error_response_scenario_names(api_scenario: Scenario):
    operation_id = get_operation_id_by_scenario(api_scenario)
    step_names = [step.name for step in api_scenario.steps]

    if "validate error" in api_scenario.name.lower():
        assert "validate error response" in api_scenario.name.lower()
        check_status_code_is_registered(api_scenario)

        info = f"{operation_id} - {api_scenario.name}"
        msg = f"[{info}] Must check error status code is registered"
        assert DEFINED_ERROR_STEP in step_names, msg


def test_always_check_status_code(api_scenario: Scenario):
    step_names = [step.name for step in api_scenario.steps]
