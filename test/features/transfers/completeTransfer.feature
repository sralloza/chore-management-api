@transfers
@transfers.complete
Feature: Transfers API - completeTransfer

    Scenario: Complete chore transfer happy path
        ...
        When a tenant completes a chore transfer with id saved as "transfer_id" using the API
    ...
    Scenario: Two users exchange tasks
        ...
        And a tenant starts a chore transfer to other tenant using the API
            | tenant_id_from | tenant_id_to | chore_type | week_id |
            | 1              | 2            | A          | 2022.01 |
        And I save the "id" attribute of the response as "transfer_id_1"
        And a tenant starts a chore transfer to other tenant using the API
            | tenant_id_from | tenant_id_to | chore_type | week_id |
            | 2              | 1            | B          | 2022.01 |
        And I save the "id" attribute of the response as "transfer_id_2"
        When a tenant completes a chore transfer with id saved as "transfer_id_1" using the API
        Then the response status code is "200"
        And the response body is validated against the json-schema "transfer"
        When a tenant completes a chore transfer with id saved as "transfer_id_2" using the API
        Then the response status code is "200"
        And the response body is validated against the json-schema "transfer"
        And the database contains the following transfers
            | tenant_id_from | tenant_id_to | chore_type | week_id | completed |
    ...
    Scenario: Validate error when completing a chore transfer twice
        Given there are 3 tenants
        And there are 3 chore types
        And I create the weekly chores for the week "2022.01" using the API
        And a tenant starts a chore transfer to other tenant using the API
    ...
    Scenario: Validate error when completing a chore transfer with invalid transfer_id
        When a tenant completes a chore transfer with id "999" using the API
