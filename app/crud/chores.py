...
class CRUDChore(CRUDBase[Chore, ChoreCreate, Chore, int]):
    async def complete_chore(
        ...
        if not chores:
            detail = f"{self.model.__name__} with week_id={week_id} and chore_type_id={chore_type_id} does not exist"
            raise HTTPException(status_code=404, detail=detail)

        if all(x.done for x in chores):
            detail = f"{self.model.__name__} with week_id={week_id} and chore_type_id={chore_type_id} is already completed"
            raise HTTPException(status_code=400, detail=detail)

        if user_id:
            user_ids = set(chore.user_id for chore in chores)
            if user_id not in user_ids:
                detail = f"You are not assigned to any chores of type {chore_type_id} for week {week_id}"
                raise HTTPException(status_code=400, detail=detail)
        else:
            print(
