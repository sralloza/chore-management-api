...
async def user_required_me_path(*, x_token: str = Header(None), user_id: str, db: AsyncSession = Depends(get_db)):
    if x_token is None:
        raise HTTPException(status_code=401, detail="Missing API key")
    if x_token == settings.admin_api_key:
        if user_id == "me":
            raise HTTPException(status_code=400, detail="Can't use the special keyword me with the admin API key")
        return
